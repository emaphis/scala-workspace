...
class ListsSuite extends munit.FunSuite:
  ...
  test("two plus one is three (0pts)?") {
   ...
   * when writing tests.
   */
  test("details why one plus one is two (0pts)") {
  ...
  test("sum of list with one number is that number") {
  ...
  test("max of list of one number is that number") {
    assertEquals(max(List(42)), 42)
  }
